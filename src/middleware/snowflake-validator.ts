 ...
 *
 * Webhook awareness: `/webhooks/{webhook_id}/{webhook_token}` is the canonical
 * webhook URL shape. The `webhook_id` segment immediately following `webhooks`
 * IS a snowflake and gets validated; the opaque `webhook_token` segment after
 * it is NOT a snowflake and is skipped (it is only scrutinized if it happens to
 * appear immediately after another resource keyword, which it does not). When
 * the path continues with `/messages/{message_id}`, the `messages` keyword
 * triggers validation of the following `message_id` segment, which IS a snowflake.
 *
 ...
 * valid snowflake. Returns a 400 response with Discord's error format
 * (`code: 50035`, `NUMBER_TYPE_COERCE`) if validation fails, preventing
 * invalid IDs from reaching the Discord API.
 */
